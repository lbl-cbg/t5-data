...
DOMAIN=`echo $JAMO_URL | sed -e 's/https:\/\///g'`

cd $HOME
jamo_status=$(curl -k -s $JAMO_URL/api/tape/diskusage | grep files)
...
# we are expecing the invocations to end with the log extension so we can have multiple prods to handle ingress
    ...
    if [ "$machine" == "dtn03.nersc.gov" ] ; then
        ...
        dt-service -D t5 -t 5 -f nersc,repo_w,hsi_1,compute,globus_1 -k ingest,delete,purge,put,copy,md5,tar,untar -r 43200 $JAMO_URL -l prod

    elif [ "$machine" == "dtn04.nersc.gov" ] ; then
        echo $(date) $0 start dt_service prod starting prep,pull
        dt-service -D t5 -t 10 -f repo_w,hsi_1,compute -k prep,pull -r 43200 $JAMO_URL -l prod
    fi
